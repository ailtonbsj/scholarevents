...
if(!isset($_GET['teste'])){
    ...
    $sqlCert = "SELECT sch_acontecimentos.titulo,sch_subeventos.titulo AS subevento FROM sch_acontecimentos,sch_subeventos WHERE (sch_acontecimentos.id_subevent = sch_subeventos.id)"
            . " AND (id_acon = $acon_id)";
    $busca_cert = $link1->query($sqlCert);
    if(!$busca_cert){
        echo "ERROR";
        exit;
    }
    $acontecimentos_d = $busca_cert->fetchAll(PDO::FETCH_ASSOC)[0];

    $sqldata = "SELECT * FROM sch_datas WHERE (label = 'Evento:Inicio') OR (label = 'Evento:Fim')";
    $buscadata = $link1->query($sqldata);
    if(!$buscadata){
        echo "ERROR";
        exit;
    }
    foreach($buscadata->fetchAll(PDO::FETCH_ASSOC) as $lndata){
        switch ($lndata['label']) {
            case "Evento:Inicio":
    ...
    $buscaHorari = $link1->query($sqlHorari);
    $totalHoras = 0.0;
    foreach($buscaHorari->fetchAll(PDO::FETCH_ASSOC) as $lnHorari){
...
foreach($buscaCert->fetchAll(PDO::FETCH_ASSOC) as $lnaCert){
